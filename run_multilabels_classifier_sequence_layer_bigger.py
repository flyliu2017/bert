...
class MultiLabelTextProcessor(DataProcessor):
    ...
    def get_dev_examples(self, data_dir):
        return self._create_examples(data_dir, "eval")
    ...
    def get_labels(self):
        """See base class."""
        ret = []
        for line in self._read_tsv(os.path.join(FLAGS.data_dir, "tag_vocab.txt")):
            ret.append(line[0])
        return ret
    ...
    def _create_examples(self, data_dir, set_type):
        """Creates examples for the training and dev sets."""
        examples = []
        with open(os.path.join(data_dir,'{}_xs_multitags'.format(set_type)), 'r', encoding='utf8') as f:
        ...
        for (i, txt) in enumerate(txts):
            guid = '%s_%s' % (set_type,i)
            txt_list=txt.split(' | ')
            text_a = tokenization.convert_to_unicode(txt_list[0])
            tags = [tokenization.convert_to_unicode(t) for t in txt_list[1:]]
            labels = tags
            examples.append(
...
class PredictTagsFromFileProcessor(MultiLabelTextProcessor):
    """Processor for the MRPC data set (GLUE version)."""

    def _create_examples(self, data_dir, set_type):
        """Creates examples for the training and dev sets."""
        examples = []
        with open(os.path.join(data_dir, '{}_xs.txt'.format(set_type)), 'r', encoding='utf8') as f:
            txts = f.read().splitlines()
        with open(os.path.join(data_dir, '{}_ys.txt'.format(set_type)), 'r', encoding='utf8') as f:
            labels = f.read().splitlines()

        for (i, n) in enumerate(zip(txts, labels)):
            txt, label = n
            guid = "%s-%s" % (set_type, i)
            text_a=' '.join(list(txt))
            label = label.split()
            examples.append(
                InputExample(guid=guid, text_a=text_a, text_b=None, label=label))
...
class PredictTagsFromPhraseProcessor(DataProcessor):
    """Processor for the MRPC data set (GLUE version)."""
    ...
    def _create_examples(self, data_dir, set_type):
        """Creates examples for the training and dev sets."""
        examples = []
        with open(os.path.join(data_dir, '{}_phrases_tags'.format(set_type)), 'r', encoding='utf8') as f:
            tags = f.read().splitlines()
        with open(os.path.join(data_dir, '{}_phrases'.format(set_type)), 'r', encoding='utf8') as f:
            phrases = f.read().splitlines()
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
    ...
    name_to_features = {
        ...
        "label_ids": tf.FixedLenFeature([322], tf.int64),
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    W_1 = tf.get_variable('dense_W1', [sequence_length,hidden_size], initializer=tf.truncated_normal_initializer(stddev=0.02))
    b_1 = tf.get_variable('dense_b1', [sequence_length], initializer=tf.zeros_initializer())
    W_2 = tf.get_variable('dense_W2', [sequence_length, num_labels], initializer=tf.truncated_normal_initializer(stddev=0.02))
    ...
    with tf.variable_scope("loss"):
        ...
        logits_1=tf.multiply(output_layer,W_1)
        logits_1=tf.reduce_sum(logits_1,-1)
        logits_1=tf.add(logits_1,b_1)
        logits_1 = tf.nn.relu(logits_1)
        input_mask = tf.cast(input_mask, tf.float32)
        logits_1=tf.multiply(logits_1,input_mask)
        logits = tf.nn.xw_plus_b(logits_1, W_2, b_2)
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(per_example_loss, label_ids, probabilities, is_real_example):
                ...
                for j, logits in enumerate(logits_split):
                    ...
                    accuracy, update_op = tf.metrics.accuracy(label_id_, logits)
                    eval_dict[str(j)] = (accuracy, update_op)
...
def main(_):
    ...
    processors = {
        ...
        'phrasetag': PredictTagsFromPhraseProcessor,
        "fromfile" : PredictTagsFromFileProcessor
    }
    ...
    if FLAGS.do_eval:
        ...
        metrics=[precision_recall_f1score(label_id,prob,threthold_num=threthold_num) for label_id,prob in zip(label_ids,probabilities)]
    ...
    if FLAGS.do_predict:
        ...
        topk_output_predict_file = os.path.join(FLAGS.output_dir, "topk_test_results.tsv")
        topk_writer = tf.gfile.GFile(topk_output_predict_file, "w")
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            ...
            for r in result:
                ...
                pred_tag_idx = [tag_idx for tag_idx, prob in enumerate(probabilities) if prob > threthold]
                if len(pred_tag_idx) == 0:
                    # tf.logging.info('************ no one > 0.5 *************')
                    pred_tag_idx = [probabilities.index(max(probabilities))]
                pred_topk_tag_idx = sorted(range(len(probabilities)), key=lambda i: probabilities[i], reverse=True)[:3]
                pred_tag = [label_list[tag_idx] for tag_idx in pred_tag_idx]
                topk_pred_tag = [label_list[tag_idx] for tag_idx in pred_topk_tag_idx]
                # pred_topk_tag = [label_list[tag_idx] for tag_idx in pred_topk_tag_idx]
                # tf.logging.info(" pred_label = %s", pred_tag)
                writer.write("{}\n".format(' '.join(pred_tag)))
                topk_writer.write("{}\n".format(' '.join(topk_pred_tag)))

                tag = [0]*len(label_list)
                for idx in pred_tag_idx:
                    tag[idx]=1
                tag_list.append(tag)
                label_ids.append(list(r['label_ids']))
