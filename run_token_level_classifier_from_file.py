...
class ExtractPhrasesProcessor(DataProcessor):
...
class ExtractPhrasesFromFileProcessor(ExtractPhrasesProcessor):
    """Processor for the MRPC data set (GLUE version)."""
    def get_examples_from_file(self, data_dir,file):
        examples = []
...
class ExtractPhrasesTagPrefixedProcessor(ExtractPhrasesFromSegmentedInputProcessor):
    ...
    def _create_examples(self, data_dir, set_type):
        """Creates examples for the training and dev sets."""
        examples = []
        with open(os.path.join(data_dir, '{}_xs_prefix.txt'.format(set_type)), 'r', encoding='utf8') as f:
            txts = f.read().splitlines()
        with open(os.path.join(data_dir, '{}_ys_converted_tag.txt'.format(set_type)), 'r', encoding='utf8') as f:
            labels = f.read().splitlines()
...
class ExtractAllPhrasesProcessor(ExtractPhrasesFromSegmentedInputProcessor):
    ...
    def _create_examples(self, data_dir, set_type):
        ...
        with open(os.path.join(data_dir, '{}_xs_multitags'.format(set_type)), 'r', encoding='utf8') as f:
            txts = f.read().splitlines()
        with open(os.path.join(data_dir, '{}_ys_multitags'.format(set_type)), 'r', encoding='utf8') as f:
            labels = f.read().splitlines()
...
def convert_single_example(ex_index, example, label_list, max_seq_length,
    ...
    label_tokens=[tokenizer.tokenize(label) for label in label_list]
    ...
    for label in label_tokens:
        ...
        for i in range(len(tokens)-length+1):
            ...
            tf.logging.info("can't find phrase  '{}' in txts.".format(label))
...
def main(_):
    ...
    processors = {
        ...
        'fromfile':ExtractPhrasesFromFileProcessor
    }
    ...
    if FLAGS.do_predict:
        predict_examples = processor.get_examples_from_file(FLAGS.data_dir,FLAGS.from_file)
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            ...
            for (i, prediction) in enumerate(result):
                ...
                tag=[1 if n>=0.5 else 0 for n in probabilities]
                tag=[0]+tag[:-1]
                ...
                # phrase=[texta[j] if probabilities[j+1]>=0.5 else ' ' for j in range(min(len(texta),128))]

                # features=predict_iterator.get_next()
            ...
            # writer.write('accuracy: {}\n'.format(count/num_written_lines))
            # writer.write('precision: {}\n'.format(sum(precision)/num_written_lines))
            # writer.write('recall: {}\n'.format(sum(recall)/num_written_lines))
            # writer.write('fscore: {}\n'.format(sum(fscore)/num_written_lines))
