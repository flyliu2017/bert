...
class PredictTagProcessor(DataProcessor):
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
    ...
    W_1 = tf.get_variable('dense_W1', [sequence_length,hidden_size],
    ...
    if FLAGS.do_predict:
        ...
        with tf.gfile.GFile(output_predict_file, "w") as writer:
            ...
            for (i, prediction) in enumerate(result):
                ...
                tag=label_list[tag_index]
                # tag=[0]+tag[:-1]

                label_ids=prediction['label_ids'][0]
                if label_ids==tag_index:
                    count+=1

                writer.write(tag+'\n')

            accuracy=count/len(predict_examples)
            writer.write("accuracy: {}".format(accuracy))
